...
/* 
 * Extantion for catalog,section component
 */


class  TravelsoftCatalogSectionComponent extends CBitrixComponent {
    ...
    public function makeFilterAdditionalParameters () {
        ...
        $prefixParam = "AFP_"; $minPrefix = "MIN_"; $maxPrefix = "MAX_"; $cpPrefix = "CATALOG_PRICE_";
        ...
        foreach ($this->arParams as $k => $v) {
            ...
            if (strpos($k, $prefixParam) === 0 && !empty($v)) {
                  ...
                  if (strpos($k, $minPrefix) === 0) {
                      $k = substr($k, strlen($minPrefix), strlen($k));
                      if ((int)$k > 0) {
                          $propFilter["><PROPERTY_" . $k][0] = trim($v);
                      } else if (strpos($k, $cpPrefix) === 0) {
                          $propFilter["><" .$k][0] = trim($v);
                      }
                  } elseif (strpos($k, $maxPrefix) === 0) {
                      $k = substr($k, strlen($maxPrefix), strlen($k));
                      if ((int)$k > 0) {
                          $propFilter["><PROPERTY_" . $k][1] = trim($v);
                      } else if (strpos($k, $cpPrefix) === 0) {
                          $propFilter["><" .$k][1] = trim($v);
                      }
                  } elseif ($k == "ID") {
